...
//
// Adapted from the stock sequential-reviewer template: agents run directly on
// the macOS host via noSandbox() because the app cannot build in a Linux
// container (see docs/adr/0001-sandcastle-in-place-not-sandboxed.md).
//
...
// Hooks run in the worktree before the agent starts each iteration.
// Pre-resolving Swift dependencies keeps the agent's first build fast.
const hooks = {
...
for (let iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
  ...
  try {
    ...
    // RGR: Red → Green → Refactor), and commits the result.
    //
    ...
    // Phase 2: Review
    //
    // A second agent reviews the diff of the branch produced by Phase 1. It
    // uses the {{BRANCH}} prompt argument to inspect the right branch, and
