...
| Dark, compact, populated | Pass | At the minimum content size, Dark retained the same single metric row and seven-column calendar. The automatic navigation rail, fixed five-bar cues, neutral em dashes, today treatment, owner-attached detail, and keyboard focus ring remained legible without clipping or horizontal scroll. |
| Light, wide, populated | Pass | A separate final Computer Use pass measured the Light window at `1180 × 720` pixels. All four exact English metric values remained in one row; all seven calendar columns, five intensity bands, neutral days, today, future days, detail surface, and legend remained visible without horizontal scroll. The accessibility tree retained `4,200 spoken words, 6 active days` and excluded July 25–31. |
